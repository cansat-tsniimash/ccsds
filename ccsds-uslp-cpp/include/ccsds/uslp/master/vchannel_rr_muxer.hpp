...
protected:
	virtual void add_vchannel_source_impl(vchannel_emitter * source) override;
...
private:
	...
	{
		const bool operator()(vchannel_emitter * source) { return source->peek(); }
	};

	typedef rr_muxer<vchannel_emitter*, vchannel_status_checker_t> muxer_t;

	uint16_t tfdf_size() const;

	muxer_t _muxer;
	vchannel_emitter * _selected_vchannel = nullptr;
